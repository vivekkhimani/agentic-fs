...
| `afs-server` | `settings`, the **pluggable store registry**, `S3ObjectStore` + `DynamoDBCatalogStore` (moto-certified), the **`FsService` read path**, the **`IngestService` + `ExtractionPipeline` write path** (lightweight `text_native`/`pdf`/`docx` rungs in-request, optional `docling` escalation, [ADR 0006](decisions/0006-extraction-normalizer-contract.md)), a **FastAPI app** (`/v1/healthz` · `/readyz` · `/me` · `fs/{ns}/{entries,stat,doc}` · `ingest/{ns}/doc` PUT+DELETE), and an **MCP mount** at `/mcp` (`whoami` · `fs_list` · `fs_stat` · `fs_read`, in-process) |
