...
| Ingest → extract → heal | `ingestion` ✅ | **Hybrid extraction** (ADR 0009): serving extracts common files **inline** (light `text_native,pdf,docx` ladder, instant) while the **S3-event worker** OCR-escalates the rest — EventBridge → SQS (+DLQ) → worker Lambda running `text_native,pdf,docx,textract,docling`, skipping rows already extracted inline. The **reconciler** (scheduled S3↔catalog heal) is the remaining piece |
